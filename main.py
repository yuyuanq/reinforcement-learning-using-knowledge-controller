...
def train():
    ...
    while True:
        ...
        if config.log_extra and update_count % config.log_extra_interval == 0:
            if config.no_controller:
                ...
                for _action in model.controller.rule_dict.keys():
                    for k, _rule in enumerate(model.controller.rule_dict[_action]):
